         ...
         <div class="row">
            <nav class="col-md-2 d-none d-md-block bg-light sidebar">
              ...
              <h2 style="padding:10px;font-size:22px;border-bottom:solid 1px  blue;">
                  <?php echo $user ?>
                  <i style="color:blue; cursor:pointer;float:right"
                     onMouseOut="this.style.color='#00F'"
                     onMouseOver="this.style.color='#0F0'"
                     class="fa fa-cog"
                     onclick="openWindowWithPostRequest('<?php echo $user ?>')">
                 </i>
              </h2>
            ...
            <main role="main" class="col-md-9 ml-sm-auto col-lg-10 pt-3 px-4">
               ...
               <div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pb-2 mb-3 border-bottom">
                  <h1 class="h2">Əməliyatlar</h1><p>(Kapital bank ödəniş kodu)  <b>4169 7414 7835 6809</b>  </p>
                  <!--
               ...
               <div class="table-responsive">
                  <table class="table table-striped table-sm">
                    ...
                    <?php $ordersor=$db->prepare("SELECT * from orders WHERE order_username=:usname order by order_id DESC");
                          $ordersor->execute(array('usname'=>$user)); $n=0;
                     ...
                     <?php } while($ordercek=$ordersor->fetch(PDO::FETCH_ASSOC)){ $n++;  ?>
                       ...
                       <div class="time"  style="display:none">
                        ...
                        <tr>
                           ...
                           <td>
                             <?php if($ordercek['order_durum'] == "Yoxlamada") {?>
                               <button onclick="ShowCheck('uploads/<?php echo $ordercek['order_imgurl']; ?>')" class="button button1">Bax</button>
                             <?php } elseif($ordercek['order_durum'] == "Gözləmədə"){ ?>
                               <button onclick="paymentWindow(<?php echo $ordercek['order_id']; ?>)" class="button button1">Ödəniş et</button>
                             <?php } elseif($ordercek['order_durum'] == "Ləğv olunub"){?>
                               <button onclick="ShowCheck('uploads/<?php echo $ordercek['order_imgurl']; ?>', '<?php echo $ordercek['order_message']; ?>')" class="button button1">Səbəb</button>
                             <?php } ?>
                           </td>

                           <td>
                             <?php if($ordercek['order_durum'] == "Yoxlamada") {?>
                               <button class="button" ><?php  minuteLeft($ordercek['order_verifytime']) ;?></button>
                             <?php } elseif($ordercek['order_durum'] == "Gözləmədə" || $ordercek['order_durum'] == "Ləğv olunub"){ ?>
                               <button onclick="removeOrder(<?php echo $ordercek['order_id']; ?>)" class="button button2">Ləğv et</button>
                             <?php } ?>
                           </td>
...
swal({
    ...
    .then((willOK) => {
        if (willOK) {
            ...
            swal("Sifariş ləğv olundu!").then((no) => {  window.location.replace(location.pathname) });
        }
    });
...
function ShowCheck(imgUrl,message){
  swal({
    title: "Aşağıdakı səbəblərdən dolayı ləğv olundu",
    text: message,
    icon: "warning",
    content: {
    element: "img",
